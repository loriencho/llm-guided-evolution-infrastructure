...
def generate_augmented_code(txt2llm, augment_idx, apply_quality_control, top_p, temperature, inference_submission=False):
    """Generates augmented code using LLM such as Mixtral, LLaMA 3, or Gemini."""
...
def extract_note(txt):
    """Extracts note labeled with # -- NOTE -- from the part if present."""
    if "# -- NOTE --" in txt:
...
def split_file(filename):
    """
    Read a file and splits it into parts separated by # -- OPTION -- 
    """
...
def llm_code_qc(code_from_llm, base_code, generate_text):
    """
    Perform quality control on the code generated by an LLM by comparing it 
    with the base code and refine by another LLM call
...
def llm_code_qc_hf(code_from_llm, base_code, generate_text=None):
    """
    Perform quality control on code (Hugging Face version)
    """
    # TODO: make parameter
...
def mutate_prompts(n=5):
    """
    Mutating prompt by submitting a rephrase request of the prompt to LLM 
    """
